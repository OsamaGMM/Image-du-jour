...
const debut = new Date(2023, 8, 29); // 15 mai 2023
const fin = new Date(2023, 10, 30); // 24 mai 2023
...
async function genererCommentairesAleatoires(idImage, jour) {
  ...
  for(let i=0; i<nbComs; i++) {
      ...
      docCom = await addDoc(collection(bd, collImages, idImage, collComs), {
          ...
          timestamp: Math.floor(Math.random()*(fin.getTime() - jour.getTime())) + jour.getTime()
      });
  }
}
